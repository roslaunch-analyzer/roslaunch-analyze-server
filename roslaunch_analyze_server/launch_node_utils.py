            ...
            if child.get('from') is not None:
                ...
                for key in data:
                    if isinstance(data[key], str) and data[key].startswith('$(var '):
                        local_parameters[key] = analyze_string(data[key], context, local_context, base_namespace)
                    else:
                        local_parameters[key] = data[key]
    context["__tree__"].add_child(context["__current_launch_name_"], f"{pkg}/{exec}", **local_parameters)
